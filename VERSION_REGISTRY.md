...
| `065f0c0` | 2026-03-24 ~16:00 UTC | pilot-expansion-tomi-leo-marcos | Adds TOMI, LEO, MARCOS to PILOT_CMS (dry-run). All three in DRY_RUN_CMS -- dashboards populate, no kills, no Slack. CM_MONITOR_CHANNELS: #cc-tomi (C0ANFLJPS69), #cc-leo (C0ANK3F1ED8), #cc-marcos (C0ANH1S3K2S). Outlook 3 workspace added (defaultCm: LEO). No logic changes. |
| `afa690a` | 2026-03-24 ~17:00 UTC | pilot-go-live-tomi-leo-marcos | DRY_RUN_CMS emptied. All 9 pilot CMs now fully live with kills enabled. Purge run triggered on deploy. Global kill cap bug: Leo consumed all 10, Tomi+Marcos deferred. |
| `6e5e426` | 2026-03-24 ~18:20 UTC | per-cm-kill-cap | Fix kill cap: per-CM budget via Map instead of single global counter. Each CM gets MAX_KILLS_PER_RUN independently. Also deleted 2,143 stale V1-era audit_logs for LEO/MARCOS/TOMI (pre-pilot, March 16-18). |
| `db4056f` | 2026-03-24 ~23:45 UTC | phase-7-self-audit | **Phase 7: Automated self-audit.** New file `self-audit.ts` (848 lines). Runs 13 deterministic checks after every eval cron: run_completion, kill_integrity, dashboard_dedup, error_regression, ghost_audit, threshold_math, kv_integrity, supabase_sync, slack_delivery, leads_monitoring, winner_detection, cross_run_consistency, daily_snapshot. Verdict: GREEN/YELLOW/RED. Writes to new `audit_results` Supabase table. Posts digest to #cc-admin (C0APDQ3MMR6). 36h auto-cleanup. New env var: `AUDIT_SLACK_CHANNEL`. No Phase 1-6 changes. Read-only for KV. Error-isolated in try/catch. CC review passed (2 rounds). |
| `93913f9` | 2026-03-25 ~15:00 UTC | self-audit-noise-fix | Fix 2 permanent WARNs: (1) `slack_delivery` short-circuits to PASS when `SLACK_SUPPRESSED=true` (per-item Slack intentionally skipped since `c6a99c7`). (2) `cross_run_consistency` workspace check uses tolerance of 1 instead of exact equality (listWorkspaces filters out keyless workspaces). No Phase 1-6 changes. No new API calls. |
| `782c0c8` | 2026-03-25 ~UTC | off-campaign-skip | OFF campaigns now fully skipped (early return before any API calls). Previously evaluated with 1.2x threshold buffer. Saves API calls and processing time. Log changed from "buffered" to "skipped". No kills, notifications, or snapshots for OFF campaigns. |
